...
class Agent:  # Do not change the name of this class!
    ...
    def heuristic_search(self, board: GameBoard) -> List[Action]:
        ...
        frontier = []
        heapq.heappush(frontier, (8,(current_position, [])))
        ...
        while frontier:
            current_pos, path = heapq.heappop(frontier)[-1]
            ...
            for action in board.get_applicable_moves(self.player):
                ...
                if action in reached and f>=reached[action]:
                    continue
                heapq.heappush(frontier,(f,count,(action, path + [MOVE(self.player,action)])))
                count +=1
                reached[action]=f
        return []
